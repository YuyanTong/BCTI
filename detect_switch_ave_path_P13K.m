clc
clear
load ALL_gene_ID_new
load COADavecasenew
load subnetsgenesall_PI3K
load subnetall_P13K

COADavecase=[COADavecasenew];
COADsubnet1genes=subnetsgenesall;
ALL_gene_ID_COAD=ALL_gene_ID_new;
COADsubnet1=subnetall;
for i=1:length(COADsubnet1genes)
    for j=1:length(ALL_gene_ID_COAD)
        if COADsubnet1genes(i)==ALL_gene_ID_COAD(j)
            the_num(i)=j;
        end
    end
end
...
BLCAcancer_data=COADavecase(the_num,:);
